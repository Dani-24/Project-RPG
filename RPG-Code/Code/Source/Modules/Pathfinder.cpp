...
Pathfinder::Pathfinder(App* application, bool start_enabled) : Module(application, start_enabled), map(NULL), lastPath(DEFAULT_PATH_LENGTH), width(0), height(0)
{
...
void Pathfinder::SetMap(uint width, uint height, uchar* data)
{
	this->width = width;
	this->height = height;
...
bool Pathfinder::CheckBoundaries(const iPoint& pos) const
{
...
uchar Pathfinder::GetTileAt(const iPoint& pos) const
{
...
const DynArray<iPoint>* Pathfinder::GetLastPath() const
{
	return &lastPath;
}
...
uint PathNode::FindWalkableAdjacents(PathList& listToFill) const
{
	...
	if (app->pathfinder->IsWalkable(cell))
...
// ----------------------------------------------------------------------------------
int Pathfinder::CreatePath(const iPoint& origin, const iPoint& destination)
{
	int ret = -1;
	int iterations = 0;
